...
(function () {
    ...
    // ── 考試使用的六款遊戲（正式考、模擬考、越級考試共用）──────────────
    // ⚠️ 這六款是作者定案。其餘遊戲（含尚未完工的 26／29／32／34／35）
    ...
    const EXAM_GAMES = [13, 20, 3, 14, 37, 33];   // 人事時地／丟三落一／字爬梯／步步驚心／步步為陣／作者是誰
    ...
    /* ------------------------------------------------------------------
     *  六款遊戲的出題比例（作者定案 2026-09-01；33 於 2026-09 加入）
     *
     *      人事時地 : 丟三落一 : 字爬梯 : 步步驚心 : 步步為陣 : 作者是誰
     *          2    :    2    :   1   :    1    :    1   :    1
     *
     ...
     *  不搶人事時地的比重），背景知識通道的考卷佔比從 2/7 微升到 3/8，
     *  仍以語感／背景為主、不被單一通道稀釋。
     *
     *  改比例只要改這張表；權重為 0 等同不出這一款（但仍留在候選清單裡，
     *  遇到某首詩只有它出得了題時，examEngine 的改派機制照樣找得到它）。
     * ---------------------------------------------------------------- */
    const EXAM_GAME_WEIGHTS = { 13: 2, 20: 2, 3: 1, 14: 1, 37: 1, 33: 1 };
