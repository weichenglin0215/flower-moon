(function () {
    ...
    const Game20 = {
        ...
        difficultySettings: {
            ...
            '研究所': { timeLimit: 10, poemMinRating: 3, maxMistakeCount: 1, formats: ['B', 'C'], minMaskCount: 4, maxMaskCount: 6, optionCount: 7, maxLines: 6, minChars: 8, maxChars: 60 }
        },
        ...
        prepareChallenge: function () {
            ...
            const result = getSharedRandomPoem(
                ...
                // minLines 由題型 A/B/C 決定，不隨難度層變動，故不放進 difficultySettings
                needLines, Math.max(needLines, settings.maxLines), settings.minChars, settings.maxChars, "",
        ...
        updateTimerRing: function (ratio, mode) {
            ...
            // ⚠️ .game20-answer-grid-container 的尺寸定義在 game20.css（loadCSS() 動態載入），
        ...
        gameOver: function (win, reason) {
            window.FMGame.gameOver(this, win, reason || '', {
                ...
                setButtons: (win) => {
                    document.getElementById('game20-retryGame-btn').disabled = win;
                    document.getElementById('game20-newGame-btn').disabled = win;
                }
            });
        }
    };
