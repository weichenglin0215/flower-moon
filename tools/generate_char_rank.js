...
// 將 Map 轉換為陣列並排序 (由大到小)
const sortedChars = Array.from(charCountMap.entries())
    .sort((a, b) => b[1] - a[1])
    .map(entry => entry[0]);

// 輸出成 .js 檔案
const outputFilePath = path.join(__dirname, '../data/最常見詩詞字排名.js');
const outputContent = `const CharacterFrequencyRank = ${JSON.stringify(sortedChars)};\n`;

fs.writeFileSync(outputFilePath, outputContent, 'utf8');

console.log(`成功匯出 ${sortedChars.length} 個常用字到 data/最常見詩詞字排名.js`);
