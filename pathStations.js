...
(function () {
    ...
    const PathStations = {
        ...
        _markExams: function (stations) {
            ...
            // ── 回填每一場考試的範圍起點 ────────────────────────────────
            //
            // ⚠️ 第一場考試（塾生）不能從 station 0 起算：那樣會把整個免試的
            //    「蒙童」區間（書僮…蒙童，連蒙童自己的必修內容）也一併塞進
            //    範圍，讓玩家第一次應試就要考全程題數最多的一場（實測：19
            //    首、38 題），恰好與「越早的考試風險越低」的教學設計相反
            //    （見上面 need=1 那段註解）。
            //
            //    起點改成「最後一個免試文位站**自己**的下一站」——只跳過
            //    免試文位站本身的必修內容（書僮、蒙童自己那一站），其後的
            //    階站（蒙童二階、準塾生…）與塾生自己仍全數列入範圍。
            //    換算下來塾生的考試範圍縮成「蒙童二階＋準塾生＋塾生」
            //    三站、9 首詩，其餘（書僮、書僮二階、準蒙童、蒙童）完全
            //    不考——蒙童仍然維持「完全免考」，只是不再被塾生的考卷
            //    間接考掉。
            //
            //    後續每一場考試的範圍起點不受影響：只要發生過一次考試，
            //    下面迴圈就會把 scopeFrom 蓋成「上一場考試站 + 1」，
            //    這裡的調整只影響「還沒發生過任何考試」的那一段。
            let lastFreeRankIdx = -1;
            for (let i = 0; i < stations.length; i++) {
                if (stations[i].type === 'rank' && !stations[i].isExam) lastFreeRankIdx = i;
            }
            let scopeFrom = lastFreeRankIdx + 1;
            for (let i = 0; i < stations.length; i++) {
